...
from jointgs.utils.graphics import fov2focal, get_projection_matrix
from jointgs.utils.rotations import batch_look_at_th
...
def get_rotating_camera(img_size=512, fov=0.4, dist=5.0, device='cuda', nframes=40, angle_limit=2*torch.pi):
    ...
    for i in range(len(w2c_mats)):
        ...
        projection_matrix = get_projection_matrix(znear=znear, zfar=zfar, fovX=fovx, fovY=fovy, device='cpu').transpose(0,1)
        full_proj_transform = (world_view_transform.unsqueeze(0).bmm(projection_matrix.unsqueeze(0))).squeeze(0)
        camera_center = world_view_transform.inverse()[3, :3]
